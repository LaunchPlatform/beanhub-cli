...
@cli.command(name="import", help="Import data into Beancount files based on the beanhub-import config file")
@click.option(
    "-c", "--config", type=click.Path(exists=True, dir_okay=False), default=".beanhub/imports.yaml", help="The path to import config file",
)
@click.option(
    "-w", "--workdir", type=click.Path(exists=True, dir_okay=True, file_okay=False), default=str(pathlib.Path.cwd()), help="The beanhub project path to work on",
)
@click.option(
    "-b", "--beanfile", type=click.Path(exists=True, dir_okay=False, file_okay=True), default="main.bean", help="The path to main entry beancount file",
)
@pass_env
def main(env: Environment, config: str, workdir: str, beanfile: str):
    ...
    parser = make_parser()
    imported_txns = list(extract_imported_transactions(parser=parser, bean_file=beanfile_path, ))
    ...
    for target_file, change_set in change_sets.items():
        if not target_file.exists():
            ...
            env.logger.info("Create new bean file %s with %s transactions", target_file, len(change_set.add))

            bean_content = "\n\n".join(map(txn_to_text, change_set.add))
            new_tree = parser.parse(bean_content)
        else:
            env.logger.info("Applying change sets (add=%s, update=%s, remove=%s) to %s", len(change_set.add), len(change_set.update), len(change_set.remove), target_file)
